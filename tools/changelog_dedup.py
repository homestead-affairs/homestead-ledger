...
title into the merge commit *body*:
    ...
    body:    feat(keep): declare the storage adapter contract

release-please parses that body, so one change reaches it twice — once as the
real commit, once as the merge commit that carried its title. The fleet saw
this on jeles 0.5.0, which listed its single change twice, from the merge and
from the commit it merged, and had to be corrected by hand.

**Ported across the fleet (willow-mcp -> jeles -> here), where the same
mechanism did worse.** In willow-mcp it also *dropped* a real commit:
release-please collapses entries that share a scope, so a release listed the
merge commit's title and lost `0073767` entirely — a shipped fix, absent from
the changelog, with a synthetic entry standing where it should have been. A
duplicate is noise; a missing entry means released work is undocumented.
Nothing about that failure is repo-specific, so homestead gets the same guard
from the start rather than waiting to be bitten by the worse half.

The tool itself is unchanged from the fleet's — it reads THIS repo's own
CHANGELOG.md, release-please-config.json and git history, so the behaviour
follows the repo it sits in.
