...
Ported into homestead-ledger from the engine (homestead-affairs), which ported it
from jeles and willow-mcp, where those four were paid for. This is the ledger's
...
constant was carried over unchanged: homestead-ledger is below 1.0 and packages
`homestead_ledger/`, not the engine's `homestead/` or willow-mcp's
...
def test_the_pr_title_check_guards_both_directions():
    ...
    `homestead_ledger`, so a copied constant would make the check pass on
    everything."""
    wf = _REPO / ".github" / "workflows" / "pr-title.yml"
    body = _yaml(wf)["jobs"]["title"]["steps"][-1]["run"]
    assert 'PACKAGED = ("homestead_ledger/", "pyproject.toml")' in body, \
    ...
    pyproject = tomllib.loads((_REPO / "pyproject.toml").read_text())
    assert pyproject["tool"]["hatch"]["build"]["targets"]["wheel"]["packages"] == ["homestead_ledger"]
