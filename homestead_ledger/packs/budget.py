...
(`tests/test_budget.py::test_the_budget_sidecar_is_not_a_discovered_pack`).
    ...
    note (free text about the limit)        → L4
...
**Why `note` mirrors `overlay.py`'s own note.** Free text about a limit can
name anything a household chooses to say, including a protected category —
unlike a closed word it cannot be held to a floor, so it is classified at
the ceiling outright, for the identical reason `packs/overlay.py` gives its
`note`.
...
SCHEMA: dict[str, Any] = {
    ...
    "note": _field(
        Rung.L4,
        "step 3: free text can name anything a household chooses to say "
        "about one category's limit, including a protected category — the "
        "same reason packs/overlay.py gives its own note.",
