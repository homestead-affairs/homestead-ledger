...
NOT_INSTALLED = (
    "nestor-meaning is not installed -- merchant resolution and reconciliation "
    "are an optional extra: pip install 'homestead-ledger[entity]'"
)
...
def available() -> bool:
    ...
    degrades to *feature absent* on ``False`` -- the books, the obligations and
    the queue never depended on Nestor and keep working without it.
    """
...
def bind(household_root: Path | None = None) -> Path | None:
    ...
    if not available():
        ...
        # ``entity`` extra still keeps its books. Nothing is bound, so every
        # Nestor-backed call still refuses with ``SeamNotBoundError`` rather
        # than reaching an unpinned ledger.
        _bound = False
        _ledger_path = None
        return None
